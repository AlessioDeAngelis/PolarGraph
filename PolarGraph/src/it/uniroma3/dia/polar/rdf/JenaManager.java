...
public class JenaManager {
	public List<RecommendedObject> textQueryEuropeana(String term) {
		...
		String endpointsSparql = "PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX edm: <http://www.europeana.eu/schemas/edm/> PREFIX ore: <http://www.openarchives.org/ore/terms/> PREFIX dc: <http://purl.org/dc/elements/1.1/>  SELECT ?proxy ?title ?subject ?mediaURL ?creator ?source WHERE {     FILTER (regex(?subject, '"+ term + "') || regex(?title, '"+ term + "')) ?resource ore:proxyIn ?proxy ; dc:title ?title ; dc:subject ?sunject; dc:creator ?creator ; dc:source ?source . ?proxy edm:isShownBy ?mediaURL .  }";
		// endpointsSparql =
